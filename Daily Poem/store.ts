...
export async function sync(): Promise<{ fetched: string[]; missing: string[] }> {
  ...
  for (let i = 0; i < AHEAD; i++) {
    ...
    try {
      ...
      Loom.log.warn(`Daily Poem: ${date} failed to download: ${e.message}`);
      missing.push(date);
      break;
    }
  ...
  if (fetched.length) Loom.log.info(`Daily Poem: downloaded ${fetched.join(', ')}`);
...
export async function prune(): Promise<number> {
  ...
  if (removed) Loom.log.info(`Daily Poem: pruned ${removed} old article(s)`);
  return removed;
}
