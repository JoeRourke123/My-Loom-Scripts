// Generic resumable "daily research article" engine. Knows nothing about poems — swap the Recipe
// (see poems.ts) and this drives any subject.
//
...
const WIKI_UA = 'LoomDailyPoem/1.0 (personal Loom automation; contact via github.com/loom)';
...
async function loadRow(t: any, date: string): Promise<Row | null> {
  ...
  Loom.log.warn(`Daily Poem: removed ${rows.length - 1} duplicate row(s) for ${date}`);
  return keep;
}
...
async function save(t: any, row: Row, mine: number, values: Row): Promise<boolean> {
  ...
  if (wrote !== 1) Loom.log.warn(`Daily Poem: lost lease mid-unit on ${row.date}, discarding result`);
  return wrote === 1;
}
...
const UNITS: Record<string, (r: Recipe, t: any, row: Row, mine: number) => Promise<void>> = {
  ...
  async illustrate(recipe, t, row, mine) {
    ...
    try {
      ...
      Loom.log.warn(`Daily Poem: image lookup failed: ${e.message}`);
    }
...
async function recordFailure(t: any, row: Row, mine: number, err: any): Promise<void> {
  ...
  Loom.log.warn(`Daily Poem ${row.date} ${row.stage}[${row.cursor}] ${kind}: ${msg}`);
  ...
  if (kind !== 'transient' && attempts >= policy.max) {
    ...
    if (policy.onExhausted === 'skip') {
      Loom.log.warn(`Daily Poem ${row.date}: skipping ${row.stage}[${row.cursor}] after ${attempts} attempts`);
...
function label(row: Row): string {
  ...
  switch (row.stage) {
    case 'pick': return 'Choosing today’s poem…';
    case 'plan': return 'Reading the poem and planning the article…';
...
export async function prune(recipe: Recipe, keepDays: number): Promise<number> {
  ...
  if (removed) Loom.log.info(`Daily Poem: pruned ${removed} unliked article(s) older than ${keepDays} days`);
  return removed;
}
