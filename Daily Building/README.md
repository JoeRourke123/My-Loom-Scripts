...
A researched feature about one building, written overnight and waiting on the phone each morning.
Widget on the home screen, the full sheet in a web view.
...
A routine runs at 05:00 and publishes `daily/building/<date>.json` to this repo (see
[`routines/building.md`](../routines/building.md)). The phone fetches it from
...
A `Today's building` notification fires once per article, scheduled for 07:00 so a 05:00 download does
not buzz at 05:00.
...
3. Add the widget: long-press home screen → **Loom → Daily Building**.
...
[`routines/building.md`](../routines/building.md) (voice, and what makes a building worth a thousand words).
