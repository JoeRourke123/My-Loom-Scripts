...
// Recipe (see artworks.ts) and this drives anything. Copied verbatim from Daily Artwork apart from
// the notification title and this header: the seam held, which was the point of writing it.
//
...
const WIKI_UA = 'LoomDailyArtwork/1.0 (personal Loom automation; contact via github.com/loom)';
...
async function loadRow(t: any, date: string): Promise<Row | null> {
  ...
  Loom.log.warn(`Daily Artwork: removed ${rows.length - 1} duplicate row(s) for ${date}`);
  return keep;
}
...
async function save(t: any, row: Row, mine: number, values: Row): Promise<boolean> {
  ...
  if (wrote !== 1) Loom.log.warn(`Daily Artwork: lost lease mid-unit on ${row.date}, discarding result`);
  return wrote === 1;
}
...
const UNITS: Record<string, (r: Recipe, t: any, row: Row, mine: number) => Promise<void>> = {
  ...
  async illustrate(recipe, t, row, mine) {
    ...
    try {
      ...
      Loom.log.warn(`Daily Artwork: image lookup failed: ${e.message}`);
    }
...
async function recordFailure(t: any, row: Row, mine: number, err: any): Promise<void> {
  ...
  Loom.log.warn(`Daily Artwork ${row.date} ${row.stage}[${row.cursor}] ${kind}: ${msg}`);
  ...
  if (kind !== 'transient' && attempts >= policy.max) {
    ...
    if (policy.onExhausted === 'skip') {
      Loom.log.warn(`Daily Artwork ${row.date}: skipping ${row.stage}[${row.cursor}] after ${attempts} attempts`);
...
function label(row: Row): string {
  ...
  switch (row.stage) {
    // Subject-neutral on purpose: this file is the reusable engine and a Recipe can be about
    // anything. Both consumers (views.ts, widget.ts) derive richer wording from status.stage.
...
export async function prune(recipe: Recipe, keepDays: number): Promise<number> {
  ...
  if (removed) Loom.log.info(`Daily Artwork: pruned ${removed} unliked article(s) older than ${keepDays} days`);
  return removed;
}
